...
<objective>
[MỤC ĐÍCH TỐI THƯỢNG]: Tệp Markdown này LÀ ĐẦU VÀO CHO HỆ THỐNG DỊCH THUẬT MÁY (Machine Translation) VÀ ĐÓNG GÓI THÀNH TÀI LIỆU MICROSOFT WORD (.DOCX). Do đó, TÍNH LIỀN MẠCH CỦA NGỮ CẢNH (Contextual Continuity), ĐỘ SẠCH của văn bản, BẢO TOÀN VỊ TRÍ ẢNH và TƯƠNG THÍCH HOÀN HẢO VỚI WORD EQUATION là ưu tiên số một.
</objective>
...
- NGOẠI LỆ: Các đoạn thơ/bài hát/khối mã nguồn (code blocks) thường có các dòng ngắn, thụt lề, lặp lại cấu trúc thì PHẢI giữ nguyên ngắt dòng, chỉ nối các đoạn văn xuôi.
...
- Dọn dẹp rác mã hóa: Tự động nhận diện và XÓA SẠCH hoặc SỬA LẠI các tàn dư của thẻ XML/HTML/MathML bị gãy vụn do lỗi OCR để trả lại văn bản nguyên vẹn.
...
- Công thức toán học & Chỉ số (Math, Sub/Superscript - Tối ưu cho Word .DOCX): 
  + BẮT BUỘC sử dụng cú pháp Toán học chuẩn **LaTeX**. Tuyệt đối KHÔNG DÙNG thẻ HTML (`<sup>`, `<sub>`) hay thẻ MathML, vì chúng sẽ gây lỗi hiển thị khi chuyển đổi sang Word Equation (OMML).
  + Đối với công thức trên cùng một dòng (Inline Math): Bọc trong MỘT dấu đô la `$`. Ví dụ, căn bậc hai của x² + y² viết là: `$\sqrt{x^2 + y^2}$`.
  + Đối với công thức đứng riêng một dòng (Display/Block Math): Bọc trong HAI dấu đô la `$$` ở đầu và cuối khối.
  + Đối với các chỉ số đơn giản (VD: E=mc², H₂O, thế kỷ 20th): Vẫn PHẢI dùng cú pháp LaTeX inline (Ví dụ: `$E=mc^2$`, `$H_2O$`, `$20^{th}$`) để trình biên dịch đồng bộ hóa toàn bộ dưới dạng Word Equation.
...
- Xử lý Ký tự đặc biệt (Escaping Characters): Dùng dấu gạch chéo ngược `\` (backslash) trước các ký tự như `#`, `*`, `[`, `>`, và đặc biệt là ký hiệu tiền tệ `\$` nếu trong tài liệu gốc chúng chỉ là văn bản thông thường (Ví dụ: `\#hashtag`, `12 \* 5`, `Giá \$50`), tránh làm trình biên dịch Markdown nhận diện nhầm thành lệnh định dạng hoặc khối công thức toán học.
